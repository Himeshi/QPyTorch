...
__device__ __inline__ uint16_t boundedPosit8ToBfloat16_gpu(uint8_t p, int rs) {
	// get sign
	bool sign = p & 0x80;
	p = (p ^ -sign) + sign;
...
__device__ __inline__ uint8_t bfloat16ToBoundedPosit8_gpu(uint16_t bf, int rs) {
  uint8_t p = 0;
	bool sign = bf & 0x8000;
	bf &= 0x7FFF;
  ...
  int regime_and_exp_length;
  uint16_t regime_and_exp;
  if((exp >> _G_ESIZE) != (rs - 1) || ((exp >> _G_ESIZE) != -rs)) {
...
__device__ __inline__ uint8_t bfloat16ToPosit8_gpu(uint16_t bf) {
  uint8_t p = 0;
	bool sign = bf & 0x8000;
	bf &= 0x7FFF;
...
void generate_bounded_posit_constants(int nsize, int es, int rs, uint32_t* int32_constants, uint64_t* int64_constants) {
  ...
  if (nsize <= 16 ) {
    ...
    _G_USEED_ZEROS = (1 << es);
    POSIT_EXPONENT_MASK = _G_USEED_ZEROS - 1;
    _G_MAXREAL_INT = ((_G_USEED_ZEROS * (rs - 1)) + SINGLE_PRECISION_BIAS) << FLOAT_EXPONENT_SHIFT;
    _G_MINREAL_INT = ((_G_USEED_ZEROS * (1 - rs)) + SINGLE_PRECISION_BIAS) << FLOAT_EXPONENT_SHIFT;
  } else {
    printf("unexpected posit config\n");
    exit(1);
  }
...
__global__ void boundedPosit8_bfloat16_kernel_nearest( uint16_t* input, uint16_t*output, float scale,  size_t input_size, int rs) {
  const int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index < input_size) {
    uint8_t temp = bfloat16ToBoundedPosit8_gpu(input[index], rs);
    output[index] = boundedPosit8ToBfloat16_gpu(temp, rs);
  }
}
...
void boundedPosit8_bfloat16_kernel_nearest_wrapper(uint16_t *__restrict__ a,
    ...
    uint32_t int32_constants_host[11];
    uint64_t int64_constants_host[2];
    generate_bounded_posit_constants(nsize, es, rs, int32_constants_host, int64_constants_host );
