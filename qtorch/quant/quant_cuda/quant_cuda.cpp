...
Tensor bfloat16_boundedPosit8_quantize_nearest(Tensor a, int nsize, int es, int rs, float scale)
{
  ...
  a = a * scale;
  a = a.view(torch::kUInt16);
  auto result = bfloat16_boundedPosit8_quantize_nearest_cuda(a, nsize, es, rs, scale);
  result = result.view(torch::kBFloat16);
  result = result / scale;
  return result;
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
  ...
  m.def("bfloat16_boundedPosit8_quantize_nearest", &bfloat16_boundedPosit8_quantize_nearest, "Low-Bitwidth Posit8 bfloat16 nearest rounding (CUDA)");
